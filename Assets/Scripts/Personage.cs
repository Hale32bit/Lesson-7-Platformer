...
public class Personage : MonoBehaviour, IPersonageControl
{
    ...
    private const float _horizontalSpeed = 2f;
    private const float _jumpImpulseSpeed = 7f;
    private const float _minJumpPeriod = 1f;
    private const float _minInclineCos = 0.9f;
    ...
    private void UpdateGrounded(Collision2D collision)
    {
        ...
        {
            if (Vector2.Dot(contact.normal, Vector2.up) > _minInclineCos)
            {
                _isGrounded = true;
                return;
            }
    ...
    void IPersonageControl.Jump()
    {
        ...
        if (Time.time - _lastJumpTime < _minJumpPeriod)
            return;

        _rigidbody.velocity += Vector2.up * _jumpImpulseSpeed;
        _isGrounded = false;
